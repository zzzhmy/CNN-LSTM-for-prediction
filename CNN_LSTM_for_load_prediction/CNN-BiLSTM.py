...
#准备数据集
#df=pd.read_csv("/data2/zmy/no_null_Residential_13.csv",parse_dates=["date"],index_col=[0])
...
a=df[['y']]
# plt.plot(a,'b') #读取到Open这一列，列名就是1
# plt.ylabel("x_value")
# plt.xlabel("time")
# plt.savefig('/data2/zmy/img/x_value.png')
...
df_ele_max=df['x'].max()
df_ele_min=df['x'].min()
print("最高负荷=", df_ele_max)
print("最低负荷=", df_ele_min)


df=df.apply(lambda x:(x-min(x))/(max(x)-min(x))) #标准化到0~1之间
#print(df)

total_len=df.shape[0] #0输出行数，1输出列数
...
for i in range(total_len-sequence):
    ...
    x.append(np.array(df.iloc[i:(i+sequence),].values,dtype=np.float32)) #取从i开始的7行 的值，
...
train_x = x_tensor[:int(0.833 * total_len)]
train_y = y_tensor[:int(0.833 * total_len)]


# 数据集前80%后的数据（20%）作为验证集
valid_x = x_tensor[int(0.833 * total_len):]
valid_y = y_tensor[int(0.833 * total_len):]
...
class cnn_bilstm(nn.Module):
    def __init__(self,window_size,feature_number):
        ...
        self.bilstm1 = nn.LSTM(input_size=64 * feature_number, hidden_size=64, num_layers=1, batch_first=True,bidirectional=True)
        #self.bilstm2 = nn.LSTM(input_size=128, hidden_size=64, num_layers=1, batch_first=True,)
...
#创建实例化网络模型
cnn_bilstm=cnn_bilstm(window_size=10,feature_number=3)
print(cnn_bilstm)
...
#定义优化器
learning_rate=0.01
opitmizer=torch.optim.Adam(cnn_bilstm.parameters(),learning_rate)
...
for i in range(epoch):
    ...
    y_train_pred=cnn_bilstm(train_x)
...
#plt.show()
plt.savefig('/data2/zmy/img/cnn_bilstm_loss.png')
...
#plt.show()
plt.savefig('/data2/zmy/img/cnn_bilstm_mapeloss.png')
fig = plt.figure() # 创建新的Figure对象
plt.plot(r2_hist, "r")
plt.grid()
plt.xlabel("times")
...
#plt.show()
plt.savefig('/data2/zmy/img/cnn_bilstm_r2.png')


y_test_pred=cnn_bilstm(valid_x)
...
with torch.no_grad():
    ...
    predicts_unnormalized = df_ele_min + (df_ele_max - df_ele_min) * predicts
    labels_unnormalized = df_ele_min + (df_ele_max - df_ele_min) * labels

    print("预测值归一化前的shape:", predicts_unnormalized.shape)
    #print("正则标准化后的预测数据：\n", predicts)
    print("")
    ...
    fig = plt.figure() # 创建新的Figure对象
    plt.grid()
    plt.plot(predicts_unnormalized,"r",linestyle="--",)
    plt.plot(labels_unnormalized,  "g")   
    plt.legend([ "pred-x",  "true-x"])
    #plt.ylim([-40, 70])  # 设置y轴范围
    #plt.show()
    plt.savefig('/data2/zmy/img/bilstm_x.png')
